 ...
 */
// Static point generator defined outside React to preserve component render purity
function generatePoints(pointCount: number, active: boolean): [Float32Array, Float32Array] {
  ...
  for (let i = 0; i < totalPoints; i++) {
    ...
    let x = 0;
    let z = 0;
    const y = Math.random() * 2.8; // room height
...
export default function PointCloudStreamer({ pointCount, active }: StreamerProps) {
  ...
  const [positions, colors] = useMemo(() => {
    return generatePoints(pointCount, active);
  }, [pointCount, active]);
